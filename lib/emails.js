...
// 'Live oak (92% confident)' when the gate passed, the plain fallback string when not.
function speciesLine(estimate) {
  const pct = estimate?.species_confidence;
  const name = estimate?.species || 'Not determinable from photos';
  return pct == null ? name : `${name} (${pct}% confident)`;
}
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111;">
  ...
  <div style="padding:28px 32px;background:#fff;border:1px solid #e5e7eb;border-top:none;">
    ...
    <p style="font-size:13px;color:#6b7280;margin:8px 0 24px;">
      Species: ${esc(speciesLine(estimate))} | ${esc(estimate.estimated_height)} | Condition: ${esc(estimate.condition)} | Complexity: ${esc(estimate.complexity)}
    </p>
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111;">
  ...
  <div style="padding:28px 32px;background:#fff;border:1px solid #e5e7eb;border-top:none;">
    ...
    <p style="font-size:13px;color:#6b7280;margin:8px 0 24px;">
      Species: ${esc(speciesLine(estimate))} | ${esc(estimate.estimated_height)} | Condition: ${esc(estimate.condition)}
    </p>
