...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111;">
  ...
  <div style="padding:14px 32px;background:#f9fafb;border:1px solid #e5e7eb;border-top:none;font-size:12px;color:#9ca3af;">
    TreeSnap &bull; AI-powered tree estimate tools for tree service companies<br>
    <a href="https://treesnap.cloud/privacy" style="color:#9ca3af;text-decoration:underline;">Privacy Policy</a>
  </div>
</div>`;
}
...
// Sent to the PREVIOUS email address whenever an account email is changed, so
// an unauthorized change (e.g. someone phoning/emailing support to swap it) is
// caught by the real account owner.
export async function sendEmailChangedNotice(oldEmail, newEmail, customer = {}) {
  return getResend().emails.send({
    from: FROM,
    to: oldEmail,
    replyTo: REPLY_TO,
    subject: 'Your TreeSnap account email was changed',
    html: wrap(`
      <h2 style="margin:0 0 16px;">Your account email was changed</h2>
      <p>${customer.owner_name ? `Hi ${customer.owner_name},` : 'Hello,'}</p>
      <p>The email address on your TreeSnap account${customer.company_name ? ` for <strong>${customer.company_name}</strong>` : ''}
         was just changed from this address (<strong>${oldEmail}</strong>) to <strong>${newEmail}</strong>.</p>
      <p>From now on, your login, billing receipts, and lead notifications will go to the new address.</p>
      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:6px;padding:16px 20px;margin:20px 0;">
        <strong style="color:#b91c1c;">Didn't request this change?</strong><br>
        If you did <strong>not</strong> ask us to update your email, please contact us
        <strong>immediately</strong> at
        <a href="mailto:hello@treesnap.cloud" style="color:#b91c1c;">hello@treesnap.cloud</a>
        so we can secure your account and reverse the change.
      </div>
      <p style="color:#6b7280;font-size:14px;">This notice was sent to your previous email address on file as a security precaution.</p>
    `),
  });
}

// Sent to the NEW email address confirming the change completed successfully.
export async function sendEmailChangeConfirmation(newEmail, customer = {}) {
  return getResend().emails.send({
    from: FROM,
    to: newEmail,
    replyTo: REPLY_TO,
    subject: 'Your TreeSnap account email was updated',
    html: wrap(`
      <h2 style="margin:0 0 16px;">Email updated</h2>
      <p>${customer.owner_name ? `Hi ${customer.owner_name},` : 'Hello,'}</p>
      <p>This confirms that the email address on your TreeSnap account${customer.company_name ? ` for <strong>${customer.company_name}</strong>` : ''}
         is now <strong>${newEmail}</strong>.</p>
      <p>Your login, billing receipts, and lead notifications will be sent to this address going forward.</p>
      <p style="color:#6b7280;font-size:14px;">Didn't expect this change? Contact us right away at
         <a href="mailto:hello@treesnap.cloud" style="color:#166534;">hello@treesnap.cloud</a>.</p>
    `),
  });
}

// Sent to the customer's current address whenever an admin makes a change to
// their account, listing each changed setting with its old → new value.
// `changes` is an array of { label, from, to } strings.
export async function sendAccountChangeSummary(toEmail, customer = {}, changes = []) {
  const rows = changes.map(c => `
    <tr>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;font-weight:600;">${esc(c.label)}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;color:#9ca3af;">${esc(c.from)}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;color:#166534;">${esc(c.to)}</td>
    </tr>`).join('');
  return getResend().emails.send({
    from: FROM,
    to: toEmail,
    replyTo: REPLY_TO,
    subject: 'Your TreeSnap account was updated',
    html: wrap(`
      <h2 style="margin:0 0 16px;">Your account was updated</h2>
      <p>${customer.owner_name ? `Hi ${customer.owner_name},` : 'Hello,'}</p>
      <p>The following ${changes.length === 1 ? 'change was' : 'changes were'} made to your TreeSnap
         account${customer.company_name ? ` for <strong>${esc(customer.company_name)}</strong>` : ''}:</p>
      <table style="width:100%;border-collapse:collapse;font-size:14px;margin:16px 0;">
        <thead><tr>
          <th style="text-align:left;padding:8px 12px;border-bottom:2px solid #e5e7eb;font-size:12px;color:#6b7280;">Setting</th>
          <th style="text-align:left;padding:8px 12px;border-bottom:2px solid #e5e7eb;font-size:12px;color:#6b7280;">From</th>
          <th style="text-align:left;padding:8px 12px;border-bottom:2px solid #e5e7eb;font-size:12px;color:#6b7280;">To</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
      <p>These changes are already live on your account.</p>
      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:6px;padding:16px 20px;margin:20px 0;">
        <strong style="color:#b91c1c;">Didn't request these changes?</strong><br>
...
// Sent to the customer just before their account is torn down by an admin.
export async function sendAccountClosedNotice(customer = {}) {
  return getResend().emails.send({
    from: FROM,
    to: customer.email,
    replyTo: REPLY_TO,
    subject: 'Your TreeSnap account has been closed',
    html: wrap(`
      <h2 style="margin:0 0 16px;">Your account has been closed</h2>
      <p>${customer.owner_name ? `Hi ${customer.owner_name},` : 'Hello,'}</p>
      <p>Your TreeSnap account${customer.company_name ? ` for <strong>${esc(customer.company_name)}</strong>` : ''}
         has been closed${customer.subdomain ? `, and your estimate tool at <strong>${esc(customer.subdomain)}.treesnap.cloud</strong> is no longer active` : ''}.
         Any active subscription has been canceled.</p>
      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:6px;padding:16px 20px;margin:20px 0;">
        <strong style="color:#b91c1c;">Didn't request this?</strong><br>
        If you did not ask to close your account, contact us immediately at
        <a href="mailto:hello@treesnap.cloud" style="color:#b91c1c;">hello@treesnap.cloud</a>.
      </div>
      <p style="color:#6b7280;font-size:14px;">We're sorry to see you go — just reply to this email if there's anything we can do.</p>
    `),
  });
}
...
// 'Species: Live oak (92% confident) | ' when the gate passed, nothing when it
// did not. A homeowner does not need to read a non-answer, and an arborist
// already knows a bare trunk is not identifiable.
function speciesSegment(estimate) {
  const pct = estimate?.species_confidence;
  if (pct == null || !estimate?.species) return '';
  return `Species: ${esc(estimate.species)} (${pct}% confident) | `;
}

export async function sendLeadNotificationEmail(customer, lead, estimate, photoUrls = [], opts = {}) {
  const serviceLabels = {
  ...
  const demoBanner = demoBannerHtml(opts.demoCopy, 'business');

  return getResend().emails.send({
    from: FROM,
    to: customer.email,
    replyTo: lead.email,
    subject: `${demoSubjectPrefix(opts.demoCopy, 2, 'your copy')}New lead: ${lead.name} — ${serviceLabel}`,
    html: `
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111;">
  ${demoBanner}
  <div style="background:#166534;padding:20px 32px;">
    <span style="color:#fff;font-weight:700;font-size:18px;">New Lead — ${esc(businessName)}</span>
  </div>
  <div style="padding:28px 32px;background:#fff;border:1px solid #e5e7eb;border-top:none;">
    <h2 style="margin:0 0 16px;font-size:16px;">Customer Info</h2>
    <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
      <tr><td style="padding:6px 0;color:#6b7280;width:120px;">Name</td><td style="padding:6px 0;font-weight:600;">${esc(lead.name)}</td></tr>
      <tr><td style="padding:6px 0;color:#6b7280;">Phone</td><td style="padding:6px 0;"><a href="tel:${esc(lead.phone)}" style="color:#166534;font-weight:600;">${esc(lead.phone)}</a></td></tr>
      <tr><td style="padding:6px 0;color:#6b7280;">Email</td><td style="padding:6px 0;"><a href="mailto:${esc(lead.email)}" style="color:#166534;">${esc(lead.email)}</a></td></tr>
      <tr><td style="padding:6px 0;color:#6b7280;">Zip Code</td><td style="padding:6px 0;">${esc(lead.zip)}</td></tr>
      <tr><td style="padding:6px 0;color:#6b7280;">Service</td><td style="padding:6px 0;">${esc(serviceLabel)}</td></tr>
    </table>
    ...
    <p style="font-size:13px;color:#6b7280;margin:8px 0 24px;">
      ${speciesSegment(estimate)}${esc(estimate.estimated_height)} | Condition: ${esc(estimate.condition)} | Complexity: ${esc(estimate.complexity)}
    </p>
    ...
    <div style="background:#fef2f2;border:1px solid #fca5a5;border-radius:6px;padding:12px 16px;margin-bottom:20px;">
      ...
      <ul style="margin:8px 0 0;padding-left:20px;color:#dc2626;">
        ${estimate.safety_concerns.map(c => `<li>${esc(c)}</li>`).join('')}
      </ul>
    </div>` : ''}
    <p style="color:#374151;"><strong>AI Notes:</strong> ${esc(estimate.notes)}</p>
    ${photosHtml}
  </div>
  <div style="padding:14px 32px;background:#f9fafb;border:1px solid #e5e7eb;border-top:none;font-size:12px;color:#9ca3af;">
    Sent by TreeSnap &bull; Preliminary estimate only — subject to in-person assessment<br>
    <a href="https://treesnap.cloud/privacy" style="color:#9ca3af;text-decoration:underline;">Privacy Policy</a>
  </div>
</div>`,
  });
...
// Demo-only labelling.
//
// SAFETY: these are the ONLY place a demo marker is produced. Both require the
// flag to be EXACTLY boolean true, so a stray truthy value ('false', 1, {}) can
// never leak a marker into a real tenant's email. Callers must derive the flag
// from isDemoHost(); never hardcode true.
// ---------------------------------------------------------------------------
export function demoSubjectPrefix(demo, n, who) {
  return demo === true ? `[TreeSnap demo ${n}/2 - ${who}] ` : '';
}

export function demoBannerHtml(demo, kind) {
  if (demo !== true) return '';
  const copy = {
    homeowner: "<strong>Demo email 1 of 2: your customer's copy.</strong> This is what the " +
               'homeowner receives after they submit photos. Check your inbox for email 2 of 2, ' +
               'which is the one that would come to you.',
    business:  '<strong>Demo email 2 of 2: your copy.</strong> This is what lands in your inbox ' +
               'the moment someone asks for a price. On your own TreeSnap page this goes to you ' +
               'and never to the customer.',
  }[kind];
  return `
  <div style="padding:14px 20px;background:#fffbeb;border:1px solid #fcd34d;border-bottom:none;border-radius:8px 8px 0 0;color:#78350f;font-size:13px;line-height:1.5;">
    ${copy}
  </div>`;
}
...
const MONTHLY_FEES = { starter: 79,  pro: 129, proplus: 179 };

export async function sendOnboardingApprovalEmail(lead, onboardingUrl) {
  return getResend().emails.send({
    from:    FROM,
    to:      lead.email,
    replyTo: REPLY_TO,
    subject: 'Your TreeSnap application is approved — one more step',
    html: wrap(`
      <h2 style="margin:0 0 16px;">Great news — you're approved!</h2>
      <p>Before we send you to payment, we need a few details about your business so your estimate tool is tuned to your actual pricing from day one.</p>
      <div style="text-align:center;margin:28px 0;">
        <a href="${onboardingUrl}"
           style="display:inline-block;background:#16a34a;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700;font-size:16px;">
          Configure My Account →
        </a>
      </div>
      <p style="font-size:12px;color:#6b7280;">This link is unique to your account. It expires in 72 hours.</p>
    `),
  });
}
...
export async function sendLeadAcknowledgmentEmail(lead) {
  return getResend().emails.send({
    ...
    html: wrap(`
      ...
      <p style="color:#6b7280;font-size:14px;">Questions? Email us at <a href="mailto:hello@treesnap.cloud" style="color:#166534;">hello@treesnap.cloud</a></p>
    `),
  });
}
  ...
  return getResend().emails.send({
    ...
    html: wrap(`
      ...
      <table style="width:100%;border-collapse:collapse;">
        <tr><td style="padding:6px 0;color:#6b7280;width:140px;">Name</td><td style="padding:6px 0;font-weight:600;">${esc(lead.name)}</td></tr>
        <tr><td style="padding:6px 0;color:#6b7280;">Company</td><td style="padding:6px 0;">${esc(lead.company)}</td></tr>
        <tr><td style="padding:6px 0;color:#6b7280;">Email</td><td style="padding:6px 0;">${esc(lead.email)}</td></tr>
        <tr><td style="padding:6px 0;color:#6b7280;">Phone</td><td style="padding:6px 0;">${esc(lead.phone)}</td></tr>
        <tr><td style="padding:6px 0;color:#6b7280;">Subdomain</td><td style="padding:6px 0;">${esc(lead.subdomain)}.treesnap.cloud</td></tr>
        <tr><td style="padding:6px 0;color:#6b7280;">Tier</td><td style="padding:6px 0;">${tierLabel(lead.tier)} \$${MONTHLY_FEES[lead.tier] ?? 79}/mo (\$${SETUP_FEES[lead.tier] ?? 299} setup)</td></tr>
        ${lead.zip ? `<tr><td style="padding:6px 0;color:#6b7280;">Zip</td><td style="padding:6px 0;">${esc(lead.zip)}</td></tr>` : ''}
      </table>
      <p style="margin-top:20px;color:#6b7280;font-size:13px;">
        Review in the <a href="https://admin.treesnap.cloud" style="color:#166534;">admin panel</a>.
      </p>
    `),
  });
}
  ...
  return getResend().emails.send({
    ...
    html: wrap(`
      ...
      <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;padding:16px 20px;margin:20px 0;">
        ...
        <strong>Plan:</strong> ${tierLabel(lead.tier)} — ${fmtMoney(monthly)}/month after 14-day free trial<br>
...
export async function sendUpgradeCheckoutEmail(customer, targetTier, checkoutUrl, upgradeFee) {
  ...
  return getResend().emails.send({
    ...
    html: wrap(`
      ...
      <p style="font-size:13px;color:#6b7280;">Questions? Email us at <a href="mailto:hello@treesnap.cloud" style="color:#166534;">hello@treesnap.cloud</a></p>
    `),
  });
}

export async function sendHomeownerEstimateEmail(homeowner, estimate, customer, opts = {}) {
  const serviceLabels = {
    removal: 'Tree Removal', trimming: 'Trimming / Pruning',
    storm_damage: 'Storm Damage Cleanup', emergency: 'Emergency Service',
  };
  ...
  return getResend().emails.send({
    ...
    subject: `${demoSubjectPrefix(opts.demo, 1, 'customer copy')}Your tree estimate from ${businessName}`,
    html: `
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111;">
  ${demoBannerHtml(opts.demo, 'homeowner')}
  <div style="background:#166534;padding:24px 32px;border-radius:8px 8px 0 0;">
    ...
    <p style="color:#bbf7d0;margin:6px 0 0;font-size:14px;">${esc(businessName)}</p>
  </div>
  <div style="padding:28px 32px;background:#fff;border:1px solid #e5e7eb;border-top:none;">
    <p style="margin:0 0 20px;">Hi ${esc(homeowner.name.split(' ')[0])}, here's your preliminary estimate for <strong>${esc(serviceLabel)}</strong>.</p>
    ...
    <p style="font-size:13px;color:#6b7280;margin:8px 0 24px;">
      ${speciesSegment(estimate)}${esc(estimate.estimated_height)} | Condition: ${esc(estimate.condition)}
    </p>
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;padding:16px 20px;margin-bottom:20px;">
      <p style="margin:0;font-size:14px;color:#166534;">${esc(estimate.notes)}</p>
    </div>
    ...
    <div style="text-align:center;margin-top:8px;">
      ...
      ${businessPhone ? `<a href="tel:${esc(businessPhone)}" style="display:inline-block;background:#166534;color:#fff;padding:12px 32px;border-radius:6px;text-decoration:none;font-weight:600;font-size:16px;">Call ${esc(businessPhone)}</a>` : ''}
      ${businessEmail ? `<p style="margin:12px 0 0;font-size:13px;color:#6b7280;">Or email: <a href="mailto:${esc(businessEmail)}" style="color:#166534;">${esc(businessEmail)}</a></p>` : ''}
    </div>
  </div>
  <div style="padding:14px 32px;background:#f9fafb;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;font-size:12px;color:#9ca3af;text-align:center;">
    Powered by TreeSnap &bull; AI-generated preliminary estimate<br>
    <a href="https://treesnap.cloud/privacy" style="color:#9ca3af;text-decoration:underline;">Privacy Policy</a>
  </div>
</div>`,
  });
