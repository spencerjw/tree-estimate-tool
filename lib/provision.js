...
const MONTHLY_PRICE_IDS = {
  starter: process.env.STRIPE_PRICE_MONTHLY_STARTER,
  pro:     process.env.STRIPE_PRICE_MONTHLY_PRO,
  proplus: process.env.STRIPE_PRICE_MONTHLY_PROPLUS,
};
...
export async function provisionCustomer(lead) {
  ...
  // 4. Stripe subscription with 14-day trial
  const trialEnd   = Math.floor(Date.now() / 1000) + 14 * 24 * 60 * 60;
  const priceId    = MONTHLY_PRICE_IDS[lead.tier];
  if (!priceId) throw new Error(`No monthly price configured for tier "${lead.tier}". Set STRIPE_PRICE_MONTHLY_${lead.tier.toUpperCase()} in Vercel env vars.`);
