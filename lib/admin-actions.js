...
// Permanently remove a lead (rejected, stale-approved, or spam). Sends no email.
// Provisioned leads are refused — they belong to a live customer and are torn
// down together with it via deleteCustomer.
export async function deleteLead(req, res) {
  ...
  if (lead.status === 'provisioned') {
    return res.status(400).json({ error: 'This lead is provisioned — delete the customer instead (that also removes the lead).' });
  }
  // Safety net for a stale status: never orphan a live customer's originating lead.
  const { data: existing } = await supabase
    .from('customers').select('id').eq('subdomain', lead.subdomain).maybeSingle();
  if (existing) {
    return res.status(409).json({ error: `Subdomain "${lead.subdomain}" belongs to an active customer — delete the customer instead.` });
  }
...
export async function editConfig(req, res) {
  ...
  const disclaimer = config.custom_disclaimer
    ...
    : null;
  // Market label ("Central Texas") — drives the tenant page tagline; blank → generic copy.
  const market = config.market
    ? String(config.market).trim().slice(0, 40) || null
    : null;
  ...
  diff('Service Area Name',   textF(o.market),               textF(market));
...
export async function deleteCustomer(req, res) {
  ...
  // Notify the customer before we tear everything down (while we still have
  // their email/subdomain). Best-effort.
  try {
    await sendAccountClosedNotice(customer);
    results.closed_email = 'sent';
  } catch (e) {
    results.closed_email = `error: ${e.message}`;
  }
