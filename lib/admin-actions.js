...
export async function approveLead(req, res) {
  ...
  const { error: updateErr } = await supabase
    .from('leads').update({ status: 'approved', onboarding_token: token }).eq('id', leadId);
...
export async function resendOnboarding(req, res) {
  ...
  const { error: updateErr } = await supabase
    .from('leads')
    .update({ onboarding_token: token, onboarding_completed_at: null, created_at: new Date().toISOString() })
    .eq('id', leadId);
  if (updateErr) return res.status(500).json({ error: updateErr.message });
